...
_QUETTA_AUTO_TOOL = Tool(
    ...
    description=(
        ...
        "  • **파일 없이** 논문 검색 (PubMed/arXiv/Semantic Scholar) → quetta_paper_search\n"
        "  • **파일(PDF) 제공** 논문 분석 (Nougat+Gemini+Claude) → quetta_analyze_paper\n"
        "    · `file_path` 또는 `file_id` 가 있으면 검색 키워드라도 자동으로 분석으로 전환\n"

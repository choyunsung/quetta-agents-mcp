...
### Windows 자동 시작 (재부팅 후에도 동작)

원격 PC가 재부팅되어도 에이전트가 자동 실행되도록 3가지 방식 제공:

#### 방식 1: Windows Service (권장 — 진짜 서비스, 로그인 불필요)

**관리자 권한 PowerShell**에서 한 줄:
```powershell
iwr -useb https://raw.githubusercontent.com/choyunsung/quetta-agents-mcp/master/install-service.ps1 | iex
```

스크립트가 자동으로:
- NSSM 다운로드 + `~/.quetta-remote-agent/nssm.exe`에 저장
- `QuettaRemoteAgent` Windows Service 등록 (자동 시작)
- `pythonw.exe agent.py` 실행, 환경변수 주입
- 로그: `~/.quetta-remote-agent/service.log`
- 실패 시 5초 후 자동 재시작
- 기존 Startup VBS / 사용자 세션 pythonw 정리

확인:
```powershell
sc.exe query QuettaRemoteAgent
# STATE : 4 RUNNING
```

#### 방식 2: 사용자 Task Scheduler (관리자 권한 불필요)

```powershell
iwr -useb https://raw.githubusercontent.com/choyunsung/quetta-agents-mcp/master/install-task.ps1 | iex
```

- `QuettaRemoteAgent` 사용자 task 등록 (로그온 + 시스템 시작 트리거)
- 실패 시 자동 재시작
- **단점**: 사용자 로그인 필요 → "자동 로그온" 함께 설정 권장:
  ```powershell
  netplwiz   # 사용자 → "사용자 이름과 암호를 입력해야" 체크 해제
  ```

#### 방식 3: Startup 폴더 VBS (가장 단순, 로그인 후만 동작)
...
### v0.14.1 (최신)
- **Windows 자동 시작 3가지 방식 추가**:
  - `install-service.ps1` — NSSM 기반 진짜 Windows Service (관리자 권한 필요, 로그인 불필요)
  - `install-task.ps1` — 사용자 Task Scheduler (관리자 불필요, 로그온 트리거)
  - 기존 Startup VBS (가장 단순)
- 모두 한 줄 `iwr | iex` 설치 가능
- 실패 시 자동 재시작, 로그 파일 회전

### v0.14.0 — 워크스페이스 멀티 테넌트
